...
DB_PATH = os.path.join(SCRIPT_DIR, 'data', 'db', 'quick_trading.db')
...
def get_positions():
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            if current_price:
                position_value = row['amount'] * current_price
                entry_value = row['amount'] * row['entry_price']
                unrealized_pnl = (position_value - entry_value) * row['leverage']
                unrealized_pnl_pct = (unrealized_pnl / (row['amount'] * row['entry_price'])) * 100
            else:
                unrealized_pnl = 0
            ...
            positions.append({
                'symbol': row['symbol'],
                'quantity': row['amount'],  # amount -> quantity
                'entry_price': row['entry_price'],
                'current_price': current_price,
                'entry_time': row['open_time'],  # open_time -> entry_time
                'leverage': row['leverage'],
                'stop_loss': row['stop_loss'] if row['stop_loss'] else 0,
                'take_profit': row['take_profit'] if row['take_profit'] else 0,
                'cost': row['amount'] * row['entry_price'],  # 计算cost
                'unrealized_pnl': unrealized_pnl,
                'unrealized_pnl_pct': unrealized_pnl_pct,
                'direction': row['direction']  # 添加方向字段
            })
