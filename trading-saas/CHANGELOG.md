...
### [5111-参数] min_score 70 → 80 (SHORT 门槛提高)

- **变更**：`auto_trader_v6.py` 的 `self.min_score` 从 70 提到 80
- **背景**：联合回测发现 70-79 分信号质量不佳，过滤后期望保留更优交易
- **效果**（基于 2025 全年回测对比）：交易数减少 8% (3340→3068)、手续费省 $134，但总亏损基本不变 (-$9608 vs -$9604)。证明问题不在信号质量而在策略赔率结构
- **部署**：服务器 139.162.31.86 已重启 auto-trader-v6 生效
- **当前状态**：余额 $9610 (-$390 from $10000 initial)

### [5111-工具] V6 策略 2025 全年完整回测系统

- **背景**：用户希望验证 v6 策略在 2025 全年的真实表现
- **新建工具**（位于 `backtest_2025/`）：
  1. `fetch_klines.py` — 从 Binance 拉 116 币种 2025 全年 1H K 线（97 万根）
  2. `backtest_v6.py` — 单币独立回测（每币 10000U）
  3. `backtest_v6_multi.py` — **联合回测**（10000U 共享，20 仓全局，与线上一致）
  4. `generate_charts.py` — 每笔交易 K 线图（开仓/止损线/移动止盈线/平仓）
  5. `web_viewer.py` — Flask Web 前端（部署至 5115 端口）
  6. `generate_reports.py` — summary.md + trades.csv + per_symbol.csv

- **关键发现（联合回测，min_score=70）**：
  - 总 PnL **-$9608 (-96%)**，最大回撤 **96.2%**
  - 1 月亏 $4432, 2 月亏 $3641，资金枯竭后 3 月起策略失活
  - 胜率 60% 但赔率 1:2.1（止盈均 +$11.91 vs 止损均 -$25.06）→ 数学负期望
  - LONG 是绝对亏损源：-$8930 (1466笔) vs SHORT -$678 (1874笔)

- **数据库**：`backtest_2025_multi.db` (含每笔交易开仓信号快照 + OHLC 持仓路径)
- **前端访问**：http://139.162.31.86:5115/
- **本地备份**：`/Users/hongtou/newproject/trading-saas/backtest_2025/`
