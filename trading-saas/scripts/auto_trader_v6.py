...
class AutoTraderV6:
    def __init__(self):
        ...
        self.enable_trend_filter = True  # MA slope趋势过滤(与回测v6对齐)
        self.long_ma_slope_threshold = 0.02
        self.ma_slope_threshold = 0.01
        ...
        # === 状态 ===
        self.positions = {}
        self.cooldowns = {}  # per-symbol cooldown (只冷却平仓的那个币)
    ...
    def _restore_capital(self):
        ...
        # Restore cooldowns from recent closed trades
        try:
            conn = sqlite3.connect(self.db_path)
            c = conn.cursor()
            ...
            for row in c.fetchall():
                ...
                if et:
                    ...
                    if elapsed < self.cooldown_seconds:
                        # Per-symbol cooldown
                        if sym not in self.cooldowns:
                            self.cooldowns[sym] = close_time
                            remaining = int((self.cooldown_seconds - elapsed) / 60)
                            print(f"  冷却恢复: {sym} 平仓{int(elapsed/60)}m前，还需等{remaining}m")
            conn.close()
        except Exception as e:
            print(f"  冷却恢复失败: {e}")
    ...
    def analyze_signal(self, symbol):
        ...
        try:
            ...
            # 趋势过滤: MA20斜率与方向冲突时跳过 (与回测v6对齐)
            if self.enable_trend_filter and len(closes) >= 25:
                ma20_now = sum(closes[-20:]) / 20
                ma20_prev = sum(closes[-25:-5]) / 20
                ma20_slope = (ma20_now - ma20_prev) / ma20_prev if ma20_prev > 0 else 0
                if direction == 'LONG' and ma20_slope < -self.long_ma_slope_threshold:
                    return 0, None
                if direction == 'SHORT' and ma20_slope > self.ma_slope_threshold:
                    return 0, None
    ...
    def calc_position_size(self, score):
        ...
        if size < 100:
            return 0, 0
        return max(100, int(size)), leverage
    ...
    def check_position(self, symbol, pos):
        try:
            ...
            should_close = False
            reason = ""
            exit_price = price  # 默认用市价

            if hold_min > self.max_hold_minutes:
                should_close = True
                reason = f"timeout {hold_min/60:.0f}h ROI={roi:+.1f}%"

            # 止损: ROI <= -10% → 用精确止损价退出 (与回测_check_position_bar对齐)
            if not should_close and roi <= self.roi_stop_loss:
                should_close = True
                sl_pct = self.roi_stop_loss / (lev * 100)
                if direction == 'LONG':
                    exit_price = ep * (1 + sl_pct)
                else:
                    exit_price = ep * (1 - sl_pct)
                reason = f"SL hit ROI={roi:+.1f}% (limit={self.roi_stop_loss}%) exit@{exit_price:.6f}"

            # 移动止盈: 用精确trail退出价 (与回测对齐)
            if not should_close and peak_roi >= self.roi_trailing_start:
                ...
                if roi <= trail_exit_roi:
                    should_close = True
                    trail_pct = trail_exit_roi / (lev * 100)
                    if direction == 'LONG':
                        exit_price = ep * (1 + trail_pct)
                    else:
                        exit_price = ep * (1 - trail_pct)
                    if trail_exit_roi > 0:
                        reason = f"trail TP @{exit_price:.6f} peakROI={peak_roi:+.1f}% exitROI={trail_exit_roi:+.1f}%"
                    else:
                        reason = f"trail SL @{exit_price:.6f} peakROI={peak_roi:+.1f}% exitROI={trail_exit_roi:+.1f}%"
    ...
    def close_position(self, symbol, exit_price, reason):
        try:
            ...
            self.cooldowns[symbol] = datetime.now()  # 该币种冷却1h
    ...
    def scan_market(self):
        ...
        for sym in self.watch_symbols:
            if sym in self.skip_coins or sym in self.positions:
                continue
            # Per-symbol cooldown check (防止同币种重复开仓)
    ...
    def _is_candle_close(self):
        """检查当前是否在1H K线收盘附近（整点后0~2分钟）"""
        now = datetime.now()
        return now.minute < 2

    def run(self, check_interval=60):
        """主循环: 止损检查每60秒, 开仓扫描仅在1H K线收盘时
        与回测对齐: 回测每根1H K线评估一次信号，实盘也只在整点扫描开仓
        """
        print(f"\n>>> 开始运行 (止损检查每{check_interval}秒, 开仓扫描每1H整点)\n")
        last_scan_hour = -1
        ...
        while True:
            try:
                ...
                # === 低频: 开仓扫描 (仅在1H K线收盘时, 与回测对齐) ===
                current_hour = datetime.now().hour
                if self._is_candle_close() and current_hour != last_scan_hour:
                    last_scan_hour = current_hour
                    self.check_risk()
                    self.scan_market()
                    self.save_snapshot()
                    pnl = self.current_capital - self.initial_capital
                    print(f"\n  === [v6] 整点扫描: 余额${self.current_capital:.2f} 盈亏${pnl:+.2f} 持仓{len(self.positions)} ===\n")
...
if __name__ == '__main__':
    # PID锁: 防止重复启动
    import fcntl
    lock_file = '/tmp/auto_trader_v6.lock'
    fp = open(lock_file, 'w')
    try:
        ...
        print("错误: auto_trader_v6 已在运行! 请先停止旧进程。")
        exit(1)
