...
def close_trade(trade_id, exit_price, reason):
    ...
    # Send close notification to Telegram group
...
def parse_signal_message(text):
    ...
    # Entry price patterns (简体+繁体) — support "市價0.31-0.307" range format
    entry_match = re.search(r'(?:入場|入场|entry|開倉|开仓|價格|价格|price|進場|进场|市價|市价)[:\s：-]*(\d+\.?\d*)\s*[-~]\s*(\d+\.?\d*)', text, re.IGNORECASE)
    if entry_match:
        # Take average of range as entry price
    ...
    # Fallback entry: "附近" pattern — e.g. "69000-70000附近空" or "67000附近多"
    ...
    # Need at minimum: symbol + direction + (sl or tp or entry_price)
...
def send_tg_notification(signal, raw_message=None, trade_info=None):
    """Send new signal + auto-opened trade notification to Telegram group"""
    try:
        ...
        current_price_str = f"<code>{current_price}</code>" if current_price else "获取失败"

        entry = signal.get('entry_price')
        entry_str = f"<code>{entry}</code>" if entry else current_price_str

        text = f"📡 <b>新信号 · 已自动开仓</b>\n"
        text += f"━━━━━━━━━━━━━━━\n"
        text += f"💰 货币: <b>{sym}</b>\n"
        text += f"📊 方向: <b>{direction_emoji}</b>\n"
        text += f"🎯 开仓价: {entry_str}\n"
        text += f"📈 当前价: {current_price_str}\n"
        ...
        if trade_info:
            text += f"💵 仓位: <code>{trade_info.get('size', '—')} USDT</code>\n"
...
def send_tg_close_notification(trade, reason, pnl_info):
    """Send trade close (TP/SL/manual) notification to Telegram group"""
    try:
        ...
        text += f"📤 平仓价: <code>{pnl_info.get('exit_price', trade.get('exit_price', '—'))}</code>\n"
        text += f"⚡ 杠杆: <code>{trade['leverage']}x</code>\n"
        text += f"💵 仓位: <code>{trade['position_size']} USDT</code>\n"
        text += f"━━━━━━━━━━━━━━━\n"
            ...
            async def handler(event):
                ...
                if signal:
                    ...
                    if auto_activate_signal(signal_id):
                        ...
                        # Get trade size for notification
                        c2 = get_db()
