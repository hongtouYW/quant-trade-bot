...
import { useVideoSEO, BASE_URL } from "@/hooks/useSEO";
...
export default function VideoPlayer() {
  ...
  const state = useVideoPlayerState(id);
  ...
  const actorNames =
    videoContent?.actor?.map((a) => a.name).filter(Boolean) || [];
  const tagNames = videoContent?.tags?.map((t) => t.name).filter(Boolean) || [];
  ...
  useVideoSEO({
    title: videoContent?.mash_title || videoContent?.title || "视频播放",
    description:
      videoContent?.description ||
      `${videoContent?.mash_title || ""} - 在线观看高清视频${actorNames.length > 0 ? `，主演：${actorNames.join("、")}` : ""}`,
    keywords: tagNames.length > 0 ? tagNames.join(",") : undefined,
    canonicalUrl,
    ogImage: videoContent?.thumb,
    ogUrl: canonicalUrl,
    videoName: videoContent?.mash_title || videoContent?.title,
    videoDuration: videoContent?.duration,
    videoThumbnail: videoContent?.thumb,
    videoUploadDate: videoContent?.publish_date,
    videoActors: actorNames,
    videoPublisher: videoContent?.publisher?.name,
    videoRating: videoContent?.rating_avg,
    videoViewCount: videoContent?.play,
  });
  ...
  // Function to get the appropriate sidebar component based on navigation source
  ...
  if (state.isPending) return <VideoPlayerSkeleton />;
