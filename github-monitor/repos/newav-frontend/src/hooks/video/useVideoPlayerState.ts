...
export function useVideoPlayerState(videoId: string | undefined) {
  ...
  // Data fetching
  const { data: videoInfo, isPending, isError } = useVideoInfo(videoId!);
  ...
  return {
    ...
    // Loading states
    isPending: isPending || isAccessPending,
    isError,
    isCollecting,
