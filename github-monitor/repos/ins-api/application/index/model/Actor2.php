...
class Actor2 extends Model
{
    ...
    public function getIsSubscribeAttr($val,$data){
        $is_subscribe = 0;
        $count = ActorSubscribe2::where('uid','=',getUid())->where('actor_id','=',$data['id'])->count();
        if($count){
            $is_subscribe = 1;
        }
        return $is_subscribe;
    ...
    public static function lists($page,$limit,$keyword){
        ...
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'actor2_list_'.$page.'_'.$limit.'_'.$keyword.'_'.$lang;
    ...
    public static function info($aid){
        $redis = new Redis();
        $lang =  getLang();
        $redis_key = 'actor2_info_'.$aid.'_'.$lang;
        $results = $redis->get($redis_key);
        if(!$results){
