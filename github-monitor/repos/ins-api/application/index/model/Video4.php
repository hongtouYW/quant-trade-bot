...
{
    protected $table = 'video4k';

    const list_field = 'id,title,title_en,title_ru,actor,publisher,thumb,preview,private,play,play_day,play_week,play_month,subtitle,source';
    const info_field = 'id,title,title_en,title_ru,mash,tags,actor,thumb,preview,panorama,video_url,description,description_en,description_ru,private,play,publisher,publish_date,subtitle,zimu,source,zimu_en,zimu_ru';
    const inter_field = 'a.id,a.title,a.title_en,a.title_ru,a.actor,a.publisher,a.thumb,a.preview,a.private,a.play,a.play_day,a.play_week,a.play_month,a.subtitle,a.source';
    const domain_path = "/4k";

    protected $hidden = ['title_en','title_ru','description_en','description_ru','zimu_en','zimu_ru'];
    protected $append = ['collect_count'];


    public function getIsCollectAttr($val,$data){
        $is_collect = 0;
        $count = VideoCollect4k::where('uid','=',getUid())->where('vid','=',$data['id'])->count();
        if($count){
            $is_collect = 1;
        }
        return $is_collect;
    ...
    public function getIsSubscribeAttr($val,$data){
        $is_subscribe = 0;
        $count = ActorSubscribe4k::where('uid','=',getUid())->where('actor_id','=',$data['actor'])->count();
        if($count){
            $is_subscribe = 1;
        }
        return $is_subscribe;
    ...
    public function getActorAttr($val){
        if($val) {
            $val = Actor4k::field('id,name,name_en,name_ru,image')->where('id','=',$val)->find();
        }
        return $val;
    }
    ...
    public function getPublisherAttr($val){
        if($val) {
            $val = Publisher::field('id,name,name_en,name_ru')->where('id','=',$val)->find();
        }
        return $val;
    }

    public function getCollectCountAttr($val,$data){
        $count =  VideoCollect4k::where('vid','=',$data['id'])->count();
    ...
    //         if($data['source'] == 0){
    //             $thumb_url = Configs::get('4k_thumb_url');
    //         }else{
    //             $thumb_url = Configs::get('thumb_url');
    //         }
    //         $val = $thumb_url.$data['preview'];
    //     }
    //     return $val;
    // }

    // public function getPreviewAttr($val,$data){
    //     if(!empty($val)){
    //         if($data['source'] == 0){
    //             $thumb_url = Configs::get('4k_thumb_url');
    //         }else{
    ...
    // public function getPanoramaAttr($val,$data){
    //     if(!empty($val)){
    //         if($data['source'] == 0){
    //             $thumb_url = Configs::get('4k_thumb_url');
    //         }else{
    ...
    public function getThumbAttr($val,$data){
        $val = '';
        if(!empty($data['preview'])){
            $val = self::domain_path.$data['preview'];
        }
        return $val;
    }
    ...
    public function getZimuAttr($val,$data){
        ...
        if(!empty($val)){
            if ($lang == 'en' && !empty($data['zimu_en']))
                $val = $thumb_url.$data['zimu_en'];
            else if ($lang == 'ru' && !empty($data['zimu_ru']))
                $val = $thumb_url.$data['zimu_ru'];
            else
                $val = $thumb_url.$val;
        }else{
            if ($lang == 'en')
    ...
    public function getVideoUrlAttr($val,$data){
        ...
        if(!empty($val)){
            if($data['source'] == 0){
                $video_url = Configs::get('4k_video_url');
            }else{
    ...
    public function getTagsAttr($val,$data){
        $val = [];
        if(!empty($data['tags'])) {
            $val = Tags::field('id,name,name_en,name_ru')->where('id','in',$data['tags'])->select();
        }
        return $val;
    }
    ...
    public static function info($vid)
    {
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video4_info_' . $vid.'_'.$lang;
    ...
    public static function video_url($vid){
        ...
        if($info['private'] == 1){
            ...
            $is_vip =User::is_vip3($uid);
            if(!$is_vip){
                throw new BaseException(3004);
            }
        }
        $ip = get_client_ip();
        $redis_key = 'video4_url_'.$vid.'_'.$ip;
        $redis = new Redis();
        ...
        self::setPlay($vid);
        if($uid){
            VideoPlayLog4::add_log($uid,$vid);
            self::tj_play($uid);
        }
        if(!empty($info['actor'])){
            Actor4k::up_play($info['actor']['id']);
        }
        return $url;
    ...
    public static function lists($page,$limit,$tag_id,$actor_id,$publisher_id,$keyword,$type){
        ...
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video4_list_'.$tag_id.'_'.$actor_id.'_'.$publisher_id.'_'.$keyword.'_'.$type.'_'.$page.'_'.$limit.'_'.$lang;
        $results = $redis->get($redis_key);
        if(!$results){
            $where = [];
            ...
            if($keyword){
                ...
                $where2 .= " and (instr(title, '".$keyword."') or instr(title_en, '".$keyword."') or instr(title_ru, '".$keyword."') or instr(mash, '".$keyword."'))";
            }
    ...
    public static function hotLists($page,$limit,$order){
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video4_list_hot_'.$order.'_'.$page.'_'.$limit.'_'.$lang;
        $results = $redis->get($redis_key);
        if(!$results){
            $where = "status = 1";
            switch ($order){
    ...
    public static function indexLists($page,$limit,$type){
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video4_list_index_'.$type.'_'.$page.'_'.$limit.'_'.$lang;
        $results = $redis->get($redis_key);
    ...
    public static function myCollect($uid,$page,$limit){
        $results = self::alias('a')
            ->join('video_collect4k b','b.vid = a.id','inner')
    ...
    public static function myPlayLog($uid,$page,$limit){
        $results = self::alias('a')
            ->join('video_play_log4 b','b.vid = a.id','inner')
            ->field(self::inter_field)
            ->where('b.uid','=',$uid)
            ->where('a.status','=',1)
            ->order('b.add_time desc')
