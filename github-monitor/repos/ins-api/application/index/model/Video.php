...
class Video extends Model
{
    ...
    public function getIsCollectAttr($val,$data){
        $is_collect = 0;
        $count = VideoCollect::where('uid','=',getUid())->where('vid','=',$data['id'])->count();
        if($count){
            $is_collect = 1;
        }
        return $is_collect;
    ...
    public function getIsSubscribeAttr($val,$data){
        $is_subscribe = 0;
        $count = ActorSubscribe::where('uid','=',getUid())->where('actor_id','=',$data['actor'])->count();
        if($count){
            $is_subscribe = 1;
        }
        return $is_subscribe;
    ...
    public function getPublisherNameAttr($val,$data){
    ...
    public static function getVid(){
        $vid = getInput('vid');
        $video = self::field('id,status')->where('mosaic','=',1)->where('id','=',$vid)->find();
        if(!$video){
    ...
    public static function info($vid)
    {
        ...
        if (!$results) {
            ...
            ->where('id','=', $vid)->where('mosaic','=',1)->find();
    ...
    public static function lists($page,$limit,$tag_id,$actor_id,$publisher_id,$keyword,$type){
        ...
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video_list_'.$tag_id.'_'.$actor_id.'_'.$publisher_id.'_'.$keyword.'_'.$type.'_'.$page.'_'.$limit.'_'.$lang;
        $results = $redis->get($redis_key);
        if(!$results){
            $where = [];
