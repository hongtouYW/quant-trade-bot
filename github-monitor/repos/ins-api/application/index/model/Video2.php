...
class Video2 extends Model
{
    ...
    const list_field = 'id,title,title_en,title_ru,actor,publisher,thumb,preview,private,play,play_day,play_week,play_month,subtitle';
    const info_field = 'id,title,title_en,title_ru,mash,tags,actor,thumb,preview,panorama,video_url,description,description_en,description_ru,private,play,publisher,publish_date,subtitle,zimu,zimu_en,zimu_ru';
    const inter_field = 'a.id,a.title,a.title_en,a.title_ru,a.actor,a.publisher,a.thumb,a.preview,a.private,a.play,a.play_day,a.play_week,a.play_month,a.subtitle';
    ...
    public function getIsCollectAttr($val,$data){
        $is_collect = 0;
        $count = VideoCollect1::where('uid','=',getUid())->where('vid','=',$data['id'])->count();
        if($count){
            $is_collect = 1;
        }
        return $is_collect;
    ...
    public function getIsSubscribeAttr($val,$data){
        $is_subscribe = 0;
        $count = ActorSubscribe1::where('uid','=',getUid())->where('actor_id','=',$data['actor'])->count();
        if($count){
            $is_subscribe = 1;
        }
        return $is_subscribe;
    ...
    public function getActorAttr($val){
        if($val) {
            $val = Actors::field('id,name,name_en,name_ru,image')->where('id','=',$val)->find();
        }
        return $val;
    }
    ...
    public function getCollectCountAttr($val,$data){
        $count =  VideoCollect1::where('vid','=',$data['id'])->count();
    ...
    public static function getVid(){
        $vid = getInput('vid');
        $video = self::field('id,status')->where('mosaic','=',0)->where('id','=',$vid)->find();
        if(!$video){
    ...
    public static function info($vid)
    {
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video2_info_' . $vid.'_'.$lang;
        $results = $redis->get($redis_key);
        if (!$results) {
            $results = self::field(self::info_field)
            ->where('id','=', $vid)->where('mosaic','=',0)->find();
    ...
    public static function video_url($vid){
        ...
        $ip = get_client_ip();
        $redis_key = 'video2_url_'.$vid.'_'.$ip;
        $redis = new Redis();
        ...
        self::setPlay($vid);
        if($uid){
            VideoPlayLog2::add_log($uid,$vid);
            self::tj_play($uid);
        }
        if(!empty($info['actor'])){
            Actors::up_play($info['actor']['id']);
        }
        return $url;
    ...
    public static function lists($page,$limit,$tag_id,$actor_id,$publisher_id,$keyword,$type){
        ...
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video2_list_'.$tag_id.'_'.$actor_id.'_'.$publisher_id.'_'.$keyword.'_'.$type.'_'.$page.'_'.$limit.'_'.$lang;
        $results = $redis->get($redis_key);
        if(!$results){
            $where = [];
    ...
    public static function hotLists($page,$limit,$order){
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video2_list_hot_'.$order.'_'.$page.'_'.$limit.'_'.$lang;
        $results = $redis->get($redis_key);
        if(!$results){
            $where = "status = 1 and mosaic = 0";
            switch ($order){
    ...
    public static function indexLists($page,$limit,$type){
        $redis = new Redis();
        $lang = getLang();
        $redis_key = 'video2_list_index_'.$type.'_'.$page.'_'.$limit.'_'.$lang;
        $results = $redis->get($redis_key);
    ...
    public static function myCollect($uid,$page,$limit){
        $results = self::alias('a')
            ->join('video_collect1 b','b.vid = a.id','inner')
    ...
    public static function myPlayLog($uid,$page,$limit){
        $results = self::alias('a')
            ->join('video_play_log2 b','b.vid = a.id','inner')
            ->field(self::inter_field)
            ->where('b.uid','=',$uid)
            ->where('a.status','=',1)
            ->order('b.add_time desc')
