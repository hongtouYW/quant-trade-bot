...
interface PublisherInfoProps {
  loaderData?: PublisherInfoType | null;
}

export default function PublisherInfo({ loaderData }: PublisherInfoProps = {}) {
  const { t } = useTranslation();
  const { id = "" } = useParams();

  const initialPublisherData: ApiResponse<PublisherInfoType> | undefined = loaderData
    ? { code: 1, msg: "ok", timestamp: 0, data: loaderData }
    : undefined;
  const { data: publisherInfo, isPending, isError } = usePublisherInfo(id, initialPublisherData);
