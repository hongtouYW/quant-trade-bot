...
import type { ActorInfo as ActorInfoType } from "@/types/actor.types.ts";
import type { ApiResponse } from "@/types/api-response.ts";

interface ActressInfoProps {
  loaderData?: ActorInfoType | null;
}

export default function ActressInfo({ loaderData }: ActressInfoProps = {}) {
  const { t } = useTranslation();
  const { id = "" } = useParams();
  const { width } = useWindowSize();
  const [isBodyDataOpen, setIsBodyDataOpen] = useState(false);

  const initialActorData: ApiResponse<ActorInfoType> | undefined = loaderData
    ? { code: 1, msg: "ok", timestamp: 0, data: loaderData }
    : undefined;
  const { isPending, data: actressInfo, isError } = useActorInfo(id, initialActorData);
  ...
  // Default to false (mobile) when width is undefined during SSR.
