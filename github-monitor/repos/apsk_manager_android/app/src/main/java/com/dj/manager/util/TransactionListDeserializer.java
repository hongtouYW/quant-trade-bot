...
public class TransactionListDeserializer implements JsonDeserializer<BaseResponse<List<Transaction>>> {
    ...
    public BaseResponse<List<Transaction>> deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
        ...
        if (root.has("history") && root.get("history").isJsonArray()) {
            List<Transaction> history = gson.fromJson(
                    root.get("history"), new TypeToken<List<Transaction>>() {
                    }.getType()
            );
            response.setHistory(history);
        }
        if (root.has("historypagination") && root.get("historypagination").isJsonObject()) {
            Pagination pagination = gson.fromJson(root.get("historypagination"), Pagination.class);
            response.setHistorypagination(pagination);
        }
        return response;
    }
}
