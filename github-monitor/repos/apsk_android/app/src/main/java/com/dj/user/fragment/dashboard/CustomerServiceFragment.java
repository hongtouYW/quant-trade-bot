...
import com.dj.user.databinding.FragmentCustomerServiceBinding;
...
//            HelpCategory.TRANSFER,
    ...
    public CustomerServiceFragment newInstance(Context context) {
    ...
    private void setupToolbar() {
        ...
        toolbarBinding.textViewTitle.setText(getString(R.string.help_title));
        toolbarBinding.imageViewLeftIcon.setOnClickListener(v -> ((DashboardActivity) context).switchTab(DashboardActivity.PAGE_HOME, 0));
    }

    private void setupUI() {
        String icon = String.format("%s%s", getString(R.string.avatar_base_url), ((DashboardActivity) context).getRandomAgentImage());
        ...
        items.add(new ItemSupport(R.drawable.ic_live_agent, v -> {
            if (!StringUtil.isNullOrEmpty(supportUrl)) {
                ...
                ((DashboardActivity) context).startAppActivity(new Intent(context, WebViewActivity.class),
...
//                }
    ...
    private void getSupportUrl() {
        ...
        ((DashboardActivity) context).executeApiCall(context, apiService.getSupportUrl(request), new ApiCallback<>() {
            @Override
    ...
    private void getFAQList() {
        ...
        ((DashboardActivity) context).executeApiCall(context, apiService.getFAQList(request), new ApiCallback<>() {
            @Override
