...
import com.dj.user.databinding.DjActivityBindEmailBinding;
...
public class DJBindEmailActivity extends BaseActivity {

    private DjActivityBindEmailBinding binding;
    private Member member;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        binding = DjActivityBindEmailBinding.inflate(getLayoutInflater());
        ...
        executeApiCall(this, apiService.requestBindEmail(request), new ApiCallback<>() {
            ...
            public void onSuccess(BaseResponse<Void> response) {
                ...
                if (!StringUtil.isNullOrEmpty(otp)) {
                    Toast.makeText(DJBindEmailActivity.this, "Testing: " + otp, Toast.LENGTH_LONG).show();
                }
                Bundle bundle = new Bundle();
                bundle.putString("data", new Gson().toJson(ActionType.BIND_EMAIL));
                bundle.putString("request", new Gson().toJson(request));
                startAppActivity(new Intent(DJBindEmailActivity.this, DJBindOTPActivity.class),
