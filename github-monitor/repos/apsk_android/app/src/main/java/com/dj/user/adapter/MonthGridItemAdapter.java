...
public class MonthGridItemAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Typeface boldTypeface = ResourcesCompat.getFont(context, R.font.pingfang_sc_semi_bold);
        Typeface normalTypeface = ResourcesCompat.getFont(context, R.font.pingfang_sc_regular);
        titleTextView.setText(item.getTitle());
        titleTextView.setTextColor(ContextCompat.getColor(context, item.isSelected() ? R.color.black_0E0E0F : R.color.gray_ABB0BD));
        ...
        itemPanel.setBackgroundResource(item.isSelected() ? R.drawable.bg_selected_date : android.R.color.transparent);
        if (item.isEnabled()) {
