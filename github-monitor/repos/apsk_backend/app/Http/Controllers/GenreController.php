...
class GenreController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        if (!CheckAuthorizedEdit($authorizedUser->currentAccessToken()->tokenable_id, GetModuleID('genre_management') )) {
            return response()->json([
                'data' => [],
                'datetime' => now(),
                'status' => false,
        ...
        $validator = Validator::make($request->all(), [
            'genre_name' => 'required|string|max:255|unique:tbl_genre,genre_name',
        ]);
        if ($validator->fails()) {
            return response()->json([
                'data' => [],
        ...
        try {
            ...
            $id = DB::table('tbl_genre')->insertGetId($newdata);
            $insertedRow = DB::table('tbl_genre')->where('genre_id', $id)->first();
            if (!$insertedRow) {
                return response()->json([
                    'data' => [],
                    'datetime' => now(),
    ...
    public function edit(Request $request)
    {
        ...
        $request->merge(['genre_name' => trim($request->input('genre_name'))]);
        $validator = Validator::make($request->all(), [
            'genre_id' => 'required|string|max:255|unique:tbl_genre,genre_id',
            'genre_name' => 'required|string|max:255|unique:tbl_genre,genre_name',
        ...
        try {
            $tbl_table = DB::table('tbl_genre')->where('genre_id', $request->input('genre_id'))->first();
            if (!$tbl_table) {
                return response()->json([
                    'data' => [],
                    'datetime' => now(),
    ...
    public function delete(Request $request)
    {
        ...
        if (!CheckAuthorizedDelete($authorizedUser->currentAccessToken()->tokenable_id, GetModuleID('genre_management') )) {
            return response()->json([
                'delete' => false,
                'datetime' => now(),
                'status' => false,
