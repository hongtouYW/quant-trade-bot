...
@section('title', __('song.add_new_song'))
@section('header-title', __('song.add_new_song'))
    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header pb-0">
                <h6>{{ __('song.add_new_song') }}</h6>
            </div>
            <div class="card-body px-0 pt-0 pb-2">
                <form action="{{ route('admin.song.store') }}" method="POST" class="p-4">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('album') is-invalid @enderror" id="album" name="album" value="{{ old('album') }}" required>
                        @error('album')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="date" class="form-control @error('published_on') is-invalid @enderror" id="published_on" name="published_on" value="{{ old('published_on') }}" required>
                        @error('published_on')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="creator" class="form-label">{{ __('artist.select') }}</label>
                        <select class="form-control @error('creator') is-invalid @enderror" id="creator" name="creator">
                            <option value="">{{ __('artist.select') }}</option>
                            @foreach ($artists as $artist)
                                <option value="{{ $artist->artist_id }}" {{ old('creator') == $artist->artist_id ? 'selected' : '' }}>
                                    {{ $artist->artist_name }}
                                </option>
                            @endforeach
                        </select>
