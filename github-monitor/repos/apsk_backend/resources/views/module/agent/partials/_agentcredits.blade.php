<div class="table-responsive">
    <table class="table table-bordered table-hover table-sm">
        ...
        <tbody>
        @forelse ($agentcredits as $credit)
            <tr>
                <td>{{ $credit->created_on }}</td>
                <td>{{ __($credit->type) }}</td>
                <td>{{ __($credit->agent->agent_name) }}</td>
                <td class="text-right font-weight-bold">{{ number_format($credit->before_balance, 2) }}</td>
                ...
                <td class="text-right text-primary font-weight-bold">{{ number_format($credit->after_balance, 2) }}</td>
            </tr>
        @empty
            <tr>
