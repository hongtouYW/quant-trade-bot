...
@section('title', __('artist.add_new_artist'))
@section('header-title', __('artist.add_new_artist'))
    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header pb-0">
                <h6>{{ __('artist.add_new_artist') }}</h6>
            </div>
            <div class="card-body px-0 pt-0 pb-2">
                <form action="{{ route('admin.artist.store') }}" method="POST" class="p-4">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('artist_desc') is-invalid @enderror" id="artist_desc" name="artist_desc" maxlength="1000" style="height: 100px;">{{ old('artist_desc') }}</textarea>
                        @error('artist_desc')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
