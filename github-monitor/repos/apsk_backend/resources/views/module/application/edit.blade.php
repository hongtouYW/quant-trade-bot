...
@section('title', __('application.edit_application'))
@section('header-title', __('application.edit_application'))
    ...
    <div class="col-12">
        <div class="card mb-4">
            <div class="card-header pb-0">
                <h6>{{ __('application.edit_application') }}: {{ $application->application_name }}</h6>
            </div>
            <div class="card-body px-0 pt-0 pb-2">
                <form action="{{ route('admin.application.update', $application->application_id) }}" method="POST" class="p-4">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('application_name') is-invalid @enderror" id="application_name" name="application_name" value="{{ old('application_name', $application->application_name) }}" required readonly>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control select2 @error('type') is-invalid @enderror"
                            ...
                            @foreach ($types as $type)
                                <option value="{{ $type }}" {{ $application->type == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-control select2 @error('platform') is-invalid @enderror"
                            ...
                            @foreach ($platforms as $platform)
                                <option value="{{ $platform }}" {{ $application->platform == $platform ? 'selected' : '' }}>
                                    {{ $platform }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('version') is-invalid @enderror" id="version" name="version" value="{{ old('version', $application->version) }}" required>
                        @error('version')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        value="{{ old('version', $application->latest_version) }}" >
                    ...
                    <div class="mb-3">
                        ...
                        id="minimun_version" name="minimun_version" 
                        value="{{ old('version', $application->minimun_version) }}" >
                    ...
                    <div class="mb-3">
                        ...
                        <input type="url" class="form-control @error('download_url') is-invalid @enderror"
                               ...
                               value="{{ old('download_url', $application->download_url) }}">
                        @error('download_url')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3">
                        ...
                        <textarea class="form-control my-editor @error('changelog') is-invalid @enderror" 
                            ...
                            name="changelog" 
                            maxlength="10000" 
                            style="height: 200px;">{{ old('changelog', $application->changelog) }}</textarea>
                        @error('changelog')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="mb-3 form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="force_update" name="force_update" 
                            value="1" {{ old('force_update', $application->force_update) == 1 ? 'checked' : '' }} >
                    ...
                    <div class="mb-3 form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="status" name="status" 
                            value="1" {{ old('status', $application->status) == 1 ? 'checked' : '' }} >
                        <label class="form-check-label" for="status">{{ __('application.is_use') }}</label>
                        @error('status')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    <button type="submit" class="btn btn-primary">{{ __('application.edit_application') }}</button>
