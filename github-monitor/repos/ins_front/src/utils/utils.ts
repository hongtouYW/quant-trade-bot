...
class Util {
  ...
  constructor() {
    this.key = "0XxdjmI55ZjjqQLO3nI7gGqrBP0Vz9jS";
    this.iv = "RWf23muavY";
    this.sign_key = "NRkw0g3iJLDvw5tJ5PuVt5276z0SOuyL";
    this.suffix = "NWSdef";
    this.secretKey = "Z7tSXMjByKCFzF5tyApk8cFpjQXD8zWd";
  }
  ...
  formatImageUrl = async (
        srcValue: string,
        withSize: boolean = true,
        imageSize: string = "600x600"
    ): FormatImageUrlResult => {
        const host =
            import.meta.env.VITE_STATIC_IMAGE_HOST || "https://imgtest.mevr757.com";

        if (!srcValue) return undefined;

        let path = "";

        // 1️⃣ Extract pathname if absolute URL
        if (/^https?:\/\//i.test(srcValue)) {
            try {
                path = new URL(srcValue).pathname;
            } catch {
                return undefined;
            }
        } else {
        ...
        // 2️⃣ Normalize path (no leading slash, no double slash)
        path = path.trim().replace(/^\/+/, "");

        // 3️⃣ Build .txt URL
        const base = host.replace(/\/+$/, "");
        const txtUrl = new URL(`${base}/${path}.txt`);

        // 4️⃣ Append size param if needed
        if (withSize && !txtUrl.searchParams.has("size")) {
            txtUrl.searchParams.set("size", imageSize);
        }

        // 5️⃣ Fetch encrypted/base64 content
        const res = await u.fetchData(txtUrl.toString());
        if (!res) return undefined;

        // 6️⃣ Decrypt if required
        return res.includes("data") ? res : u.imgDecrypt(res);
    };
}

type FormatImageUrlResult = Promise<string | undefined>;
const u = new Util();
export default u;
