...
class System extends Base
{
    ...
    public function initialize()
    {
        ...
        $this->videoHandle =new \app\index\model\VideoHandle();
    }
    ...
    public function video_handle(){
        $param = input();
        $param['page'] = !empty($param['page'])?$param['page']:1;
        $param['limit'] = !empty($param['limit'])?$param['limit']:10;
        $where = [];
        $total = $this->videoHandle->where($where)->count();
        $list =  $this->videoHandle->where($where)->page($param['page'],$param['limit'])->order('id desc')->select();
        $this->assign([
            'list'  => $list,
            'total' => $total,
            'page'  => $param['page'],
