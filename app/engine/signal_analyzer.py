...
def analyze_signal_v6(symbol: str, config: dict) -> tuple:
    """V6 signal: v4.2 base scoring + MACD/ADX/BB bonus points.

    Base score (0-100): RSI(30) + MA Trend(30) + Volume(20) + Price Position(20)
    Bonus (0-25): MACD confirmation(+10) + ADX strength(+8) + BB extreme(+7)
    Total max: 125

    Uses v4.2 risk management (3x leverage, -10% SL, 6%/3% trailing).
    """
    try:
        klines = fetch_klines(symbol, '1h', 100)
        if not klines or len(klines) < 50:
        ...
        # Calculate v5-era indicators
        macd = calculate_macd(closes)
        bb = calculate_bollinger_bands(closes)
        adx = calculate_adx(klines)

        # ═══ v4.2 BASE SCORING (0-100) ═══
        ...
        # ═══ V6 BONUS SCORING (0-25) ═══

        bonus = 0

        # MACD confirmation bonus (+3/+6/+10)
        if macd:
            hist = macd['histogram']
            prev_hist = macd['prev_histogram']
            macd_val = macd['macd']
            ...
            if direction == 'LONG':
                if hist > 0 and macd_val > 0:
                    bonus += 10  # Strong LONG confirmation
                elif hist > 0 and hist > prev_hist:
                    bonus += 6   # Building momentum
                elif hist > prev_hist:
                    bonus += 3   # Improving
            else:  # SHORT
                if hist < 0 and macd_val < 0:
        ...
        # ADX trend strength bonus (+2/+5/+8)
        if adx is not None:
            if adx >= 35:
                bonus += 8   # Very strong trend
            elif adx >= 25:
                bonus += 5   # Strong trend
            elif adx >= 20:
                bonus += 2   # Moderate trend

        # BB extreme position bonus (+4/+7)
        if bb:
            pct_b = bb['percent_b']
            if direction == 'LONG':
                if pct_b < 0.1:
                    bonus += 7   # Deep oversold at lower band
                elif pct_b < 0.25:
                    bonus += 4   # Near lower band
            else:  # SHORT
                if pct_b > 0.9:
                    bonus += 7   # Deep overbought at upper band
                elif pct_b > 0.75:
                    bonus += 4   # Near upper band
